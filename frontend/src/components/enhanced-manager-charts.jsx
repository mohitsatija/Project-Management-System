...
function ProjectStatusDonutChart({ data }) {
  ...
  const projectStatusData = [
    { browser: "pending", visitors: allProjects.filter(p => p.status === 'pending').length, fill: "var(--color-pending)" },
    { browser: "inprogress", visitors: allProjects.filter(p => p.status === 'in-progress').length, fill: "var(--color-inprogress)" },
    { browser: "completed", visitors: allProjects.filter(p => p.status === 'completed').length, fill: "var(--color-completed)" },
...
function TaskStatusPieChart({ data }) {
  ...
  const activeIndex = 0; // Always show first item as active
...
export function ManagerCharts({ data }) {
  ...
  return (
    <div className="space-y-6">
      {/* Row 2: Charts and Cards Layout - Increased Height */}
      <div className="flex gap-4 h-[400px]">
        {/* Charts Section - 75% width */}
        <div className="flex-1 grid grid-cols-3 gap-4" style={{width: '75%'}}>
          {/* Weekly Tasks Chart - Wider */}
        ...
        {/* Right Column: 3 Stacked Components - 25% width */}
        <div className="flex flex-col gap-4" style={{width: '25%'}}>
          {/* Total Salary Card - Same height as header cards (h-28) */}
          ...
          {/* Total Fund Used Card - Same height as header cards (h-28) */}
          <Card className="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 shadow-md h-28">
            <CardContent className="flex flex-col justify-center items-start p-3 h-full">
              <div className="text-left w-full">
                <p className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-1">Total Fund Used</p>
          ...
          {/* Project Status Chart - Fills remaining space */}
